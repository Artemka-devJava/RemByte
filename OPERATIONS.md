...
Контейнер по умолчанию монтирует **сам `/mnt` Linux-хоста** напрямую
(docker-compose: `- /mnt:/mnt`) — то есть видит те же точки монтирования
(NAS, USB-диски и т.п.), что и хост под `/mnt`. Если задан
`FIXBYTE_BACKUP_DIR` (по умолчанию `/mnt`) — в панели работает блок
«Хранилище полных бэкапов»: кнопка «Сохранить полный бэкап в хранилище»,
список сохранённых архивов со скачиванием / восстановлением / удалением,
авто-ротация (`FIXBYTE_BACKUP_KEEP`, по умолчанию 14). Восстановление —
кнопкой «Восстановить» у нужного архива в списке.

**Путь можно выбрать прямо в панели** (поле «Каталог для бэкапов» в том же
блоке, кнопка **«Обзор»** открывает реальное дерево папок хоста — сервер
листает содержимое, а не браузер админа). Путь обязан лежать внутри `/mnt`
...
Если не хотите отдавать контейнеру весь `/mnt` хоста, а только одну папку —
замените volume `- /mnt:/mnt` на `- ${BACKUP_HOST_DIR:-./backups}:/mnt`
(см. `deploy/backups/README.md`, «Вариант 1»).
