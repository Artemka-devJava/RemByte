...
Каталог монтируется в контейнер `app` как `/mnt` (`FIXBYTE_BACKUP_DIR=/mnt` по
умолчанию). Конкретную подпапку внутри `/mnt` — например, если туда
примонтировано несколько шар/дисков — можно выбрать прямо в панели
администратора → **База данных** → «Хранилище полных бэкапов»: путь должен
лежать внутри `/mnt`, иначе сервер откажет (это единственное, что реально
примонтировано в контейнер). Там же: сохранить сейчас, скачать, восстановить,
удалить. Старые архивы удаляются автоматически, остаётся последних
`FIXBYTE_BACKUP_KEEP` штук (по умолчанию 14).
